...
def interact_model(
    ...
    with tf.Session(graph=tf.Graph()) as sess:
        ...
        write_path = "outputs/"
        gen_file = '{:%Y_%m_%d_%H_%M_%S}_{}_len_{}_seed_{}.txt'.format(datetime.utcnow(), model_name, str(length), str(seed) )
        complete_path = write_path + gen_file
        
        
        # f = open(complete_path, "a", encoding="utf-8")
        details_string = ("model_name: " + model_name + "," + " seed:" + str(seed) + "," + " samples:" + str(nsamples) + "," +
